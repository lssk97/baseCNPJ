...
class GUI():
    ...
    def __init__(self):
        ...
        #self.update_gui = sg.Window('Updater',[[sg.Output((50,20), key='-OUTPUT-')]], finalize=True)
...
def web_scrape(browser, dados) -> list:
    ...
    try:
        if not browser.abrir_navegador()
    else:
        necessario_update = list()
        erros_scraping = list()
        if dados.current_versions['receita'].append(browser.check_receita()):
            necessario_update.append('receita')
        else:
            erros_scraping.append('receita')
            
        if dados.current_versions['abecs'].append(browser.check_abecs()[0]):
            necessario_update.append('abecs')
        else:
            erros_scraping.append('abecs')
            
        if dados.current_versions['lista_cnpj'].append(browser.check_abecs()[1]):
            necessario_update.append('lista_cnpj')
        else:
            erros_scraping.append('lista_cpj')

        if erros_scraping:
            for e in erros_scraping:
                print(f'Erro no websraping : {e}')

        browser.navegador.quit()

        return necessario_update
        

def end_update(dados, updates : list):

    if updates:
        if 'receita' in updates:
        ...
        if 'lista_cnpj' in updates:
            dados.update_depara_abecs()

start_update(dados)
x = web_scrape(browser, dados)
end_update(dados, x)
